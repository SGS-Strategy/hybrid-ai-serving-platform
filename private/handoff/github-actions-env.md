...
- `.github/workflows/private-cloud-gpu-validate.yml`: standalone/reusable GPU passthrough validation workflow using isolated temporary OpenStack resources
...
3. Storage 설치는 `setup_storage=true`를 실제 backing 준비 후 선택합니다. GPU 검증은 `validate_gpu=true`로 임시 OpenStack 리소스와 VM을 생성해 passthrough를 검증하고 종료 시 삭제합니다.
