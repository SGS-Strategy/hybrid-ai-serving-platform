...
| `GITLAB_IMAGE` | GitLab Docker image | 기본 `gitlab/gitlab-ce:latest`; 필요하면 GitLab EE image로 override |
...
| `GITLAB_ROOT_PASSWORD` | GitLab root password | GitLab 설치 후 root password 지정. 없으면 `/srv/gitlab/config/initial_root_password`의 GitLab 생성 password 사용 |
...
GitLab CE 첫 부팅은 VM 성능에 따라 오래 걸릴 수 있습니다. Workflow는 container 생성, restart policy,
reverse proxy upstream 설정까지만 필수 성공 기준으로 보고, GitLab HTTP가 아직 booting이면 실패시키지 않습니다.
GitLab이 ready가 된 뒤 같은 Apply를 다시 실행하면 GPU runner 등록 단계가 이어집니다.
...
GPU worker runner 등록은 GitLab HTTP가 ready일 때 자동 실행됩니다. Workflow가 GitLab VM 안에서 짧게 쓰는 root PAT를
