...
| `GITLAB_INSTALL_ENABLED` | GitLab 설치 여부 | 기본 `true`; `false`면 GitLab VM 생성만 하고 package 설치는 건너뜀 |
| `GITLAB_DOMAIN` | GitLab domain | 기본 `git.intp.me` |
| `GITLAB_EXTERNAL_URL` | GitLab external URL | 기본 `https://git.intp.me` |
| `GITLAB_PACKAGE` | Linux package name | 기본 `gitlab-ce`; 필요하면 `gitlab-ee` |
...
| `GITLAB_URL` | GitLab instance URL | GPU worker shell runner 등록 대상 GitLab URL. 없으면 `GITLAB_EXTERNAL_URL`, 그 다음 `https://git.intp.me` |
...
| `MINIO_ROOT_PASSWORD` | MinIO root secret key | `setup_storage=true`에서 MinIO root password 지정. 없으면 workflow가 random password 생성 |
| `GITLAB_ROOT_PASSWORD` | GitLab root password | GitLab 설치 후 root password 지정. 없으면 `/etc/gitlab/initial_root_password`의 GitLab 생성 password 사용 |
| `GITLAB_RUNNER_TOKEN` | GitLab runner authentication token | 자동 생성 대신 기존 token을 강제로 쓸 때만 사용. Terraform 변수로 넣지 않음 |
...
`Private Cloud Apply`는 기본적으로 첫 `gitlab_nodes` VM에 GitLab CE를 설치합니다. 기본 접속 주소는
`https://git.intp.me`이고, GitLab VM은 내부 HTTP port 80으로만 listen합니다. 물리 서버 Caddy는
`git.intp.me` 요청을 `127.0.0.1:18083`으로 reverse proxy하고, workflow는 이 local upstream을 GitLab VM
port 80으로 연결합니다.

root password는 `GITLAB_ROOT_PASSWORD` Secret이 있으면 그 값으로 설정합니다. 없으면 GitLab package가
생성한 `/etc/gitlab/initial_root_password`를 GitLab VM에서 확인해야 합니다.
...
GPU worker runner 등록은 기본적으로 자동 실행됩니다. Workflow가 GitLab VM 안에서 짧게 쓰는 root PAT를
만들고, 공식 `POST /user/runners` API로 runner authentication token을 생성한 뒤 PAT를 폐기합니다.
`GITLAB_RUNNER_TOKEN` Secret이 있으면 자동 생성 대신 그 값을 override로 사용합니다.

| GitHub key | 기본값 | 용도 |
| --- | --- | --- |
| `GITLAB_URL` Variable | `https://git.intp.me` | GitLab instance URL |
...
| `PRIVATE_CLOUD_DNS_SERVICES` | `HA_DNS_SERVICES` | CNAME 대상 서비스 목록, 기본 `openstack,k8s,grafana,argocd,git`. Workflow는 기존 값에도 `git`을 덧붙임 |
