...
- `validate_gpu=true`로 격리된 임시 OpenStack 리소스와 GPU VM을 만들고 `lspci`/`nvidia-smi` passthrough 검증을 실행합니다.
- GPU 검증 결과와 console log tail은 GitHub Actions Step Summary에서 확인합니다.
